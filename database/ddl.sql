use basetable;
create table if not exists basetable.black_list
(
    id         bigint       not null
...
create table if not exists basetable.category
(
...
create table if not exists basetable.debate_history
(
...
create table if not exists basetable.encrypt
(
...
create table if not exists basetable.hibernate_sequence
(
    next_val bigint null
);

create table if not exists basetable.report
(
    id                bigint       not null
        primary key auto_increment,
...
create table if not exists basetable.room
(
...
create table if not exists basetable.users
(
    ...
    constraint FKdf59o2uk813ytjs4qtfksq836
        foreign key (encrypt_user_security_id) references basetable.encrypt (user_security_id)
);

create table if not exists basetable.user_category
(
    ...
    constraint FKh1fip9lpe4alrpurxqfmpftvl
        foreign key (user_id) references basetable.users (user_id),
    constraint FKjchjxphkf5owj1i5bp95g5mfs
        foreign key (category_id) references basetable.category (category_id)
);